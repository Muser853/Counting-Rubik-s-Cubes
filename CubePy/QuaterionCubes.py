...
def go(old_visit, all_visited, step):
    ...
    if len(all_visited) == len(new_old): 
        return len(all_visited), step  
    print(len(all_visited) - len(new_old), ",", step)
    go(new_old, all_visited, step + 1)
# Recursively visit new states in breadth-first search
if __name__ == "__main__":    # Initialize Rubik's Cube of size 2 with 8 quaternions representing 8 element cubes
    go(set(), {tuple(Quaternion(i, 0, 0, 0) for i in range(1, 9))}, 0)
